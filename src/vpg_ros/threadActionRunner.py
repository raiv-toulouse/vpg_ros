...
class ThreadActionRunner(Thread):
    ...
    def run(self):
        ...
        if action_to_execute == 'push':
            self.output_variables['best_pix_ind'] = np.unravel_index(np.argmax(self.push_predictions), self.push_predictions.shape)
            predicted_value = np.max(self.push_predictions)
        elif action_to_execute == 'grasp':
            self.output_variables['best_pix_ind'] = np.unravel_index(np.argmax(self.grasp_predictions), self.grasp_predictions.shape)
        ...
        print('Action: %s at (%d, %d, %d)' % (
            action_to_execute, self.output_variables['best_pix_ind'][0], self.output_variables['best_pix_ind'][1],
            self.output_variables['best_pix_ind'][2]))
        best_rotation_angle = np.deg2rad(self.output_variables['best_pix_ind'][0] * (360.0 / self.trainer.model.num_rotations))
        best_pix_x = self.output_variables['best_pix_ind'][2]
        best_pix_y = self.output_variables['best_pix_ind'][1]
        ...
        if action_to_execute == 'push':  # or action_to_execute == 'place':
            finger_width = 0.02
        ...
        if action_to_execute == 'push':
            self.trainer.executed_action_log.append(
                [0, self.output_variables['best_pix_ind'][0], self.output_variables['best_pix_ind'][1], self.output_variables['best_pix_ind'][2]])  # 0 - push
        elif action_to_execute == 'grasp':
            self.trainer.executed_action_log.append(
                [1, self.output_variables['best_pix_ind'][0], self.output_variables['best_pix_ind'][1], self.output_variables['best_pix_ind'][2]])  # 1 - grasp
        self.logger.write_to_log('executed-action', self.trainer.executed_action_log)
        # Visualize executed primitive, and affordances
        if self.save_visualizations:
            push_pred_vis = self.trainer.get_prediction_vis(self.push_predictions, self.color_heightmap, self.output_variables['best_pix_ind'])
            self.logger.save_visualizations(self.trainer.iteration, push_pred_vis, 'push')
            cv2.imwrite('visualization.push.png', push_pred_vis)
            grasp_pred_vis = self.trainer.get_prediction_vis(self.grasp_predictions, self.color_heightmap, self.output_variables['best_pix_ind'])
